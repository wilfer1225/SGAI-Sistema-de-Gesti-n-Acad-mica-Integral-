...
function mapInscripcion(row) {
  return {
    ...
    tipoInstancia: row.tipo_instancia,
    estado: row.estado,
  };
}
...
async function historialDe(legajo) {
  const { rows } = await pool.query(
    "SELECT id_materia FROM historial_academico WHERE legajo = $1 AND estado = 'Aprobada'",
    [legajo]
  );
...
async function listarPorAlumno(legajo) {
  const { rows } = await pool.query(
    "SELECT id_inscripcion, legajo, id_materia, fecha_inscripcion, tipo_instancia, estado FROM inscripcion WHERE legajo = $1 ORDER BY fecha_inscripcion DESC",
    [legajo]
  );
